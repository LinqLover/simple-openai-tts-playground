...
const fetchAndConcatenateAudio = async (textChunks, voice, apiKey, progressFn = null) => {
...
const convert = async () => {
    ...
    try {
        ...
        audioBlob = await fetchAndConcatenateAudio(textChunks, voice, apiKey, progress => {
...
const updatePricing = async () => {
    ...
    let cachedBase64 = localStorage.getItem(cacheKey);
    if (cachedBase64) {
        document.getElementById('convertBtn').innerText = "Convert to Speech (cached)";
        return;
    }
    
    const pricePerMillion = 15.00;
    const price = (text.length / 1000000) * pricePerMillion;
    const cents = price * 100;
    document.getElementById('convertBtn').innerText = `Convert to Speech (¢${cents.toFixed(2)})`;
};
...
const init = () => {
    ...
    document.getElementById('voiceSelect').addEventListener('change', updatePricing);
